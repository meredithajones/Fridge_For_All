import React from "react";

const Home = () => (
  <div className="container rounded w-75 m-5 p-5 mx-auto d-block">
    <img
      src="imgs/FFA.png"
      width="25%"
      className="img-fluid rounded float-left m-3"
      alt="Fridge For All"
    />
    <h1>Hi there!</h1>
    <p>
      Thanks for checking out Fridge For All. We hope you find what you're
      looking for! Here is where you can find out more information about the
      community fridges in Philadelphia and most importantly check the inventory
      of your local fridge!
    </p>
    <p>
      To get started, click on "Check the Fridge" above. Please note that all
      non-perishable canned should be pop-tops. If not, please consider donating
      can openers!{" "}
    </p>
  </div>
);

export default Home;
